...
function findItemById(id) {
  let rez = items.find((item) => item.id === id);
  return rez;
...
function capitalizeNames() {
  const itemsCapital = items.map((name) => name.name.toUpperCase());
...
function calculateTotalInventory() {
  ...
  // const startValue =0;
  const sumItemsInventory = items.reduce((x, item) => x + item.inventory, 0);
  return sumItemsInventory;
}
...
function calculateAllInventoryPrice() {
  ...
  const sumItemsPrice = items.reduce((x, item) => x + item.price, 0);
...
function getItemPriceByName(name) {
  ...
  const price = items.find((name) => name == name);
  return price;
}
